...
import com.fox.myday.widgets.AbstractWidgetProvider;
import com.fox.myday.widgets.DashClockWeatherExtension;
...
public class AlarmReceiver extends BroadcastReceiver {
    ...
    public class GetWeatherTask extends AsyncTask<String, String, Void> {
        ...
        protected Void doInBackground(String... params) {
            try {
                ...
                URL url = new URL("https://api.openweathermap.org/data/2.5/weather?id=" + URLEncoder.encode(sp.getString("cityId", Constants.DEFAULT_CITY_ID), "UTF-8") + "&lang="+ language +"&appid=" + API_KEY);
    ...
    class GetLongTermWeatherTask extends AsyncTask<String, String, Void> {
        ...
        protected Void doInBackground(String... params) {
            try {
                ...
                String language = Locale.getDefault().getLanguage();
                if(language.equals("cs")) { language = "cz"; }
                URL url = new URL("https://api.openweathermap.org/data/2.5/forecast?id=" + URLEncoder.encode(sp.getString("cityId", Constants.DEFAULT_CITY_ID), "UTF-8") + "&lang="+ language +"&mode=json&appid=" + API_KEY);
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                BufferedReader connectionBufferedReader = null;
                try {
                    connectionBufferedReader = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()));
    ...
    public class GetCityNameTask extends AsyncTask <String, String, Void> {
        ...
        protected Void doInBackground(String... params) {
            ...
            try {
                URL url = new URL("https://api.openweathermap.org/data/2.5/weather?q=&lat=" + lat + "&lon=" + lon + "&lang="+ language +"&appid=" + API_KEY);
